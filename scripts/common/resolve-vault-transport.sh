...
# Runs right after the artifact-upload attempts (v4, then v3 fallback).
#
# Transport modes (vault_transport output):
#   artifact-v4 - upload-artifact@v4 succeeded; downstream downloads via @v4
#   artifact-v3 - @v4 unavailable, upload-artifact@v3 succeeded; downstream via @v3
#   local       - both artifact versions unavailable (or forced); local-file transport
#
# Required env:
...
#   UPLOAD_V3_OUTCOME - steps.upload_vault_v3.outcome ('success'|'failure'|'skipped'|'')
...
case "${FORCED}" in
  ...
  ""|auto|artifact)
    if [[ "${V4}" == "success" ]]; then
      MODE="artifact-v4"
    elif [[ "${V3}" == "success" ]]; then
      MODE="artifact-v3"
    elif [[ "${FORCED}" == "artifact" ]]; then
      echo "::error::VAULT_TRANSPORT=artifact but both upload-artifact@v4 and @v3 failed; refusing to fall back to local file. Check artifact support on this runner/GHES, or use VAULT_TRANSPORT=auto (allows local fallback) or local."
      exit 1
    else
      MODE="local"
    fi ;;
